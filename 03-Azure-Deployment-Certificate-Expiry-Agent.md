...
This tutorial guides you through deploying the Securetron PKI Trust Manager Certificate Expiry Agent to the Azure Cloud environment. You will configure container app settings, registry details, ingress rules, and complete the deployment process. The Certificate Expiry Agent provides automated monitoring and notification of any certificates that are about to expire. A critical service to prevent outages!
### Azure Portal
...
Click the container app name field to specify the name for your new container app. Enter your container app name to identify the deployment such as pki-trust-manager-cert-expiry-agent
...
Enter the Image and Tag such as certexpired:latest
...
You have successfully deployed the Securetron PKI Trust Manager Certificate Expiry Monitoring Agent to Azure Cloud by configuring container settings, registry access, ingress rules, and completing the deployment. The Certificate Expiry Monitoring Agent is your primary assistance in reviewing certificates and notifying so that action can be taken to automate renewal or have the certificate renewed manually priot to an outage. For more information please visit our website at https://securetron.net or contact support.
